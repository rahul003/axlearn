...
class TestConfig():
    def __init__(self, setup, test: ModuleConfig, golden: ModuleConfig = None, inputs: dict = None, loss_fn = None, conv_output = None):
        self.test = test
...
def _topkgather_to_topk(output, expert_cap):
    ...
    print(dispatch_tensor)
    # print(output.load_balance_loss)
    # print(output.router_z_loss)
...
def _get_training_configs():
    ...
    setup_moe_topkgather = {"input_dim": 8, "hidden_dim": 32, "num_experts": 8, "num_groups": 4, "outer_batch": 1, "gating": TopKGatingGather.default_config().set(name="gating")}
    setup_moe_top2 = {"input_dim": 8, "hidden_dim": 32, "num_experts": 8, "num_groups": 4, "outer_batch": 1}

    setup_top2 = {"expert_capacity": 200, "num_experts": 8, "train_capacity_factor": 1.0}
    setup_topk = {"expert_capacity": 200, "num_experts": 8, "train_capacity_factor": 1.0}
    setup_topkg = {"expert_capacity": 10, "num_experts": 4, "train_capacity_factor": None}
    ...
    test_cfgs = [
        ...
        TestConfig(
            ...
            golden = ModuleConfig(TopKGating, "cpu"),
            inputs = dict(logits=jax.random.uniform(jax.random.PRNGKey(1), shape=(1, 1, 10, 4))),
            loss_fn = lambda x: x.combine_tensor.mean(),
            conv_output = partial(_topkgather_to_topk, expert_cap=setup_topkg["expert_capacity"]),
        ),
    ]
...
#         # Wrap fwd function in jit for single HLO.
#         @jax.jit
#         def moe_layer():
#             return F(
#                 layer,
...
#             inputs=dict(inputs=inputs),
#         )
#         outputs, _ = moe_layer_parity()

#         @jax.jit
#         def dense_layer_parity():
#             return F(
#             layer_dense,
#             is_training=False,
#             prng_key=jax.random.PRNGKey(123),
...
#             inputs=dict(inputs=inputs),
#         )
#         outputs_dense, _ = dense_layer_parity()
#         if copy_weights:
...
#         @jax.jit
#         def adaptive_load_balance_loss_inrange():
#             return F(
#                 layer,
#                 is_training=True,
#                 prng_key=jax.random.PRNGKey(123),
...
#             )
#         scale, _ = adaptive_load_balance_loss_inrange()
#         self.assertAlmostEqual(scale, 1)

#         @jax.jit
#         def adaptive_load_balance_loss_underrange():
#             return F(
#                 layer,
#                 is_training=True,
#                 prng_key=jax.random.PRNGKey(123),
...
#             )
#         scale, _ = adaptive_load_balance_loss_underrange()
#         self.assertLess(scale, 1)

#         @jax.jit
#         def adaptive_load_balance_loss_overrange():
#             return F(
#                 layer,
#                 is_training=True,
#                 prng_key=jax.random.PRNGKey(123),
...
#             )
#         scale, _ = adaptive_load_balance_loss_overrange()
...
#         @jax.jit
#         def top2_gating_with_adaptive_load_balance_loss():
#             return F(
#                 layer,
#                 is_training=True,
#                 prng_key=jax.random.PRNGKey(123),
...
#                 inputs=dict(logits=logits),
#             )
#         gating, output_collection = top2_gating_with_adaptive_load_balance_loss()
...
#             )
#         ref_x, _ = moe_layer_structure_ref()
...
#             @jax.jit
#             def moe_layer_structure_mock_prenorm():
#                 x, _ = F(
#                     layer.norm,
...
#                 x += inputs
#                 return x
#             x = moe_layer_structure_mock_prenorm()
#         elif structure == "postnorm":
#             @jax.jit
#             def moe_layer_structure_mock_postnorm():
...
#                 )
#                 return x
#             x = moe_layer_structure_mock_postnorm()
#         elif structure == "hybridnorm":
#             @jax.jit
#             def moe_layer_structure_mock_hybridnorm():
#                 x, _ = F(
#                     layer.prenorm,
#                     inputs=dict(x=inputs),
...
#                 x += inputs
#                 return x
#             x = moe_layer_structure_mock_hybridnorm()
#         elif structure == "nonorm":
#             @jax.jit
#             def moe_layer_structure_mock_nonorm():
#                 x, _ = dispatch_and_combine_fn(inputs=dict(x=inputs))
#                 return x
#             x = moe_layer_structure_mock_nonorm()
#         else:
...
#                     is_training=True,
#                 )
#             outputs_dense, _ = dense_X_layers()
